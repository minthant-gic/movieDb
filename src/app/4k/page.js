...
const fetchMovies = async (currentPage) => {
    const res = await axios.get(
        `https://yts.mx/api/v2/list_movies.json?quality=4k&page=${currentPage}&limit=24`
    );
...
const Page = () => {
    ...
    const {data, isLoading, isError} = useQuery(['movies', currentPage], () =>
        fetchMovies(currentPage)
    );

    const movies = data?.movies || [];
    const totalPages = Math.ceil(data?.movie_count / 24) || 1;
        ...
        <div>
            ...
            <div className="flex justify-center w-full h-screen">
                <div className="w-full max-w-7xl">
                    {isLoading && (
                        <div className="flex justify-center items-center mt-60 ml-12">
                        ...
                        <>
                            <div className="flex flex-wrap">
                                {movies.map((movie, index) => (
                                    <div
                                        key={index}
                                        className="w-1/2 mt-2 sm:w-1/6 md:w-1/4 lg:w-1/6"
                                    >
